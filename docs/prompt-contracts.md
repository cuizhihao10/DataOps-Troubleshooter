...
### 2.5 在线 GraphRAG 上下文契约

`{evidence_bundle}` 和 `{retrieved_paths}` 使用版本化的 `graphrag-retrieval:v1` 结构。该结构由确定性检索服务生成，不是 LLM 输出，因此不触发 Planner Prompt 版本升级；如果字段语义或评分公式发生不兼容变化，必须提升检索契约 ID，并同步更新 Planner Prompt/评测中对该上下文的解释。

```json
{
  "contract_id": "graphrag-retrieval:v1",
  "query": "...",
