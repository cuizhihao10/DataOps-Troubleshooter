...
`portfolio-eval-manifest:v2` 固定五个已经实现且有独立实测文档的评测层：GraphRAG vector/graph、
长期记忆 vector/graph、Memory off/on 端到端影响、Auditor off/on 增量安全，以及
`golden-diagnosis-eval:v1` 顶层诊断确定性回归。代码仍可读取精确四层的 v1 历史 manifest，但默认
CLI 只使用 v2。manifest 只允许引用仓库 `tests/*.py` 文件或测试节点，不接受自由 pytest flags；
运行器使用 `subprocess.run(shell=False)`。

`portfolio-eval-run:v2` 顺序执行每层，并遵守以下指标发布门禁：
