...
`planner-react:v3` 延续 v2 的 system/user 两条消息角色隔离，并在 user 消息新增版本化 checkpoint 恢复的
`session_context`。用户问题、上一轮公开报告、Evidence、历史案例和工具 Schema 均是不可信运行
数据，只能进入 user 消息；这避免把用户文本或旧报告插值进 system 后意外提升指令优先级。
...
`session_context` 为 null 或 `SessionTurnContext`，只含来源 run、上一问题、报告摘要、根因、处置、
风险、不确定性和证据引用；不含 Prompt、Thought、供应商原始输出或 embedding。Renderer 对所有
Pydantic 数据使用排序键 UTF-8 JSON。PlannerDecision Schema 不重复写入 Prompt；官方 SDK 通过
`response_format` 从 Pydantic 类型单独提交 strict JSON Schema，减少 token 和 Schema 漂移。
...
未预期编程异常仍传播。Planner 停止后的报告与审计由独立 `audited-report-workflow:v1` 接续，
...
`auditor-report:v1` 驱动独立 Auditor 审查已经生成的 `DiagnosisReport`。Auditor 只返回
...
历史案例是否确认为 confirmed 且没有覆盖实时证据；每个修复建议是否有风险等级、前置条件、
回滚和验证；高风险建议是否有证据支持其必要性；证据不足时是否明确保留 uncertainties。
...
所有运行数据只进入 user 消息并使用排序键 UTF-8 JSON。缺失 GraphRAG 明确为 `null`，空案例和
问题为 `[]`；不能为了让审计上下文看起来完整而伪造检索或规则结果。
...
5. 历史案例是否真的存在于本轮 confirmed 上下文；无根因报告是否明确 uncertainties。
...
`audited-report-workflow:v1` 的固定拓扑为：
...
草稿由 `DeterministicReportBuilder` 从现有假设、Evidence、GraphRAG path 和 solution/SOP 节点生成；
只提升拥有有效支持引用且没有反对证据的 supported/confirmed 假设。没有方案证据时只提出低风险
只读补证，不编造生产修复。
...
该契约现由 `diagnosis-resources:v1` HTTP 入口复用并持久化 run/events/checkpoint；同 session 的
上一轮公开状态可恢复。similarity/common points/differences 仍未投影进最终 DiagnosisReport。
