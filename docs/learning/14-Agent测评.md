...
3. **`stop_reason_hit_rate` = 0.667。** 跨组件水位案例八步用尽后以 `react_budget_exhausted` 结束，Golden 期望 `evidence_sufficient`。它拿满了必要工具覆盖与证据来源覆盖，属于"证据够了但根本没拿到收口那一轮"。这也是 Run G 案例 2 锚点未命中的原因：预算耗尽后报告里根本没有根因。（Run H 的 0.464 分母是 28，其中 11 条没走到收口就 `planner_provider_error`，两者不可直接比较。）**病因已由 `langgraph-react-loop:v4` 结构性闭合**：取证预算耗尽时额外发放一次不消耗步数、批次上限为 0 的收口回合（第 8 章 §8.8.9）。当初写在这里的诊断"需要在 Prompt 里强化剩余步数临界时的收口判断"**方向是错的**——那条路是用证据覆盖率换一个好看的 `stop_reason`。但 v4 目前只有单元测试证据，上面两个数字仍是 `planner-react:v8` + `langgraph-react-loop:v3` 下的最后一次实测值，**改善幅度是待测量项，不是已知结论**。
