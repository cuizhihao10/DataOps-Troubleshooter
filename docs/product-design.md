...
- **成本约束**：限制 ReAct 最多 10 步工具行动、2 跳图扩展和 1 次审计返工，均可配置。步数上限必须严格大于 Golden 集里最长的必需工具集（跨组件案例 6 个），否则真实模型的一到两步试探会直接挤掉必需取证。"只做收口"的最后一次 Planner 决策不占用这个预算：取证步数用尽时 `langgraph-react-loop:v4` 额外发放一次批次上限为 0、不消耗步数的收口回合（每次运行只发一次），让 Planner 把已收集证据写成结论。此前 8 步下已实测命中过"预算恰好用满 → 拿不到收口回合 → 根因为空 → `report_incomplete` → 一路降级"这条链，该失败面已由收口回合结构性关闭。
...
- 面向学习和面试讲解的实现指南，逐项说明技术原理、数据流、关键代码位置、设计取舍、失败路径和验证命令。
- 单元、集成、端到端和评测用例。
