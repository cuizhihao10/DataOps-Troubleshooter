...
当前已提供 `/api/v1/sessions`、message/run/event 资源和顶层 runtime lifespan factory；但还没有
LangGraph checkpoint、可靠后台执行或同 session 旧状态恢复。similarity/common points/differences
的 Prompt/报告投影和 GraphRAG `case`/`SIMILAR_TO` 注册也仍未完成。
...
写 JSONB DiagnosisRunResult 和整批事件。这样避免慢模型阻塞轮询，也阻止重复提交覆盖终态。
...
成功事件固定按 retrieval → React 原顺序 → report 原顺序 → memory 排列。payload 只投影有限枚举、
ID、计数、停止原因和审计码，不序列化 AgentState 全量、Prompt、Thought、embedding 或异常文本。

### 15.5 当前限制与验证

同一 session 目前可以保存多个独立 run，但 message 不加载上一次 AgentState，因此还不是真正追问
checkpoint。执行模式是 synchronous，没有 queued/cancelled、后台 worker、重试 API、session 列表
或分页；这些能力需要可靠持久化调度后再升级状态契约。
...
摘要和 CheckConstraint。Docker 默认 Provider disabled，因此健康接口应显示 diagnosis API disabled，
而不是冒充可执行模型诊断。
...
- LangGraph checkpoint、同 session 追问恢复和可靠后台 run worker/取消/重试。
