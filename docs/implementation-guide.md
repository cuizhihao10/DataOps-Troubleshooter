...
`portfolio-eval-manifest:v8` 把这些层与 Golden 诊断基线的 suite ID、来源契约、结果文档、PostgreSQL 前置、受限 pytest target 和
已审核指标快照集中到 `data/evals/portfolio_eval_manifest.json`。它只汇总现有实测，不把不同层计算
成一个没有统计意义的总准确率。

`app/evaluation/portfolio.py` 实现 `portfolio-eval-run:v8`。测试 target 必须是仓库内 `tests/*.py`
文件或测试节点，manifest 不能加入任意 flags；真实执行使用当前 Python 解释器和
`subprocess.run(shell=False)`，stdout/stderr 被捕获后只在失败层提供截断摘要。运行器不会输出环境变量、
数据库 URL、Prompt、Thought 或供应商响应体。默认文件和 CLI 使用五层 v8；v1 精确四层与 v2/v3/v4/v5/v6/v7
Golden v1/v2/v3/v4/v5/v6 来源仍可兼容读取，旧结果不会被静默解释为新完整运行。

调用链如下：

```text
python -m app.evaluation
  -> load and validate portfolio-eval-manifest:v8
  ...
  -> render portfolio-eval-run:v8 JSON to stdout
```
...
`app/evaluation/golden_diagnosis.py` 实现 `golden-diagnosis-eval:v7`。它依赖可替换异步 runner，但评分
...
`golden-case:v6` 的 `case_category` 当前为 4/3/1/3/3。14 条案例复用 6 个 Fixture；三条记忆案例
...
当前 manifest 是四层小样本消融加一层 14 条 Golden 确定性回归，不是产品规划中的完整 28 条诊断
Golden Cases。Golden 报告固定 `target_case_count=28`、`case_coverage_rate=14/28` 和
...
- `golden-case:v6` 跨组件类别门禁、记忆与成功响应证据冲突标注，以及 `golden-diagnosis-eval:v7` 十四条案例评测。
- `portfolio-eval-manifest:v8` 五层受限 pytest 入口、v1–v7 兼容读取、指标发布门禁，以及 `portfolio-eval-run:v8` 单命令 JSON 汇总。
