...
模型供应商适配器不能绕过图节点。OpenAI-compatible Chat Provider、v3 Prompt Renderer 和一次
...
### 11.2 v3 Prompt 的 system/user 隔离与追问上下文

`planner-react:v3` 使用两个独立文件：system 只包含角色、安全和输出行为；user 承载查询、同会话
上一轮公开报告上下文、计划、capability、工具 Evidence、GraphRAG Bundle、路径、confirmed 案例、
允许工具和预算。session_context 与 query 都是低优先级 JSON 数据，不会插值到 system 消息。
...
可靠后台执行仍未完成。similarity/common points/differences 的 Prompt/报告投影和 GraphRAG
`case`/`SIMILAR_TO` 注册也仍未完成。
...
`diagnosis-resources:v1` 因此先采用同步执行：POST message 创建 running run，释放创建事务，完成
...
- Planner v3 双消息 Prompt、同会话公开报告上下文、OpenAI-compatible Structured Outputs Provider 与一次 Schema 修复。
...
- `diagnosis-resources:v1` session/message/run/event PostgreSQL 资源 API、同步真实执行和安全失败事件。
...
- 已确认案例 GraphRAG `case`/`SIMILAR_TO` 注册，以及历史共同点/差异点生成。
