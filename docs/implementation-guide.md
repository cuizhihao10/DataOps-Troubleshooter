...
文档只描述已经进入仓库并通过验证的实现。尚未完成的模型级 Embedding Provider、可靠后台执行和
案例到 GraphRAG 的自动注册会明确标记为后续工作，避免把设计当作完成结果。
...
任何强依赖不满足时，应用启动失败，而不是在用户提交诊断后才暴露配置错误。未配置数据库的纯单元测试模式会明确返回 `database_status=disabled`；Docker 演示模式必须返回 `database_status=ok`、`knowledge_nodes_embedded=33`，并公开不含凭据的 Provider、维度和评分权重快照。
...
`graph-seed:v8` 在 v1–v7 的 30 节点/35 边基础上，增加客户分群 LTS/BDS 两个任务节点和一个
显式数据集节点，并新增两个 RUNS_ON、DEPENDS_ON、PRODUCES、CONSUMES、MANIFESTS_AS 共六条边，
当前合计 33 节点/41 边。旧节点和边继续保留各自 v1–v7 source；只有客户分群任务拓扑使用
`synthetic_cross_chain_knowledge_v8`，并通过 MANIFESTS_AS 接入 v3 的长尾症状→数据倾斜→再平衡方案链。
...
和 history matcher 图邻居合并已经接通；尚未实现的是删除案例 API 和模型级复杂语义解释。确定性
matcher 不应被宣传成通用自然语言理解。
...
现在已经合并图邻居并把同批候选交给 Planner/Auditor；模型级复杂语义对比和历史解释评测仍未
完成。当前确定性规则只覆盖可客观比较的字段，不宣称理解任意自然语言冲突。

## 15. 资源化诊断 API

### 15.1 为什么首版选择同步执行而不是进程内后台任务

产品允许首版通过 GET run/events 轮询，但“可轮询资源”不等于必须先引入任务队列。FastAPI
`create_task` 或 BackgroundTasks 在进程重启后无法恢复，也没有跨 worker 所有权、取消和重试语义；
把它包装成 queued run 会让作品演示看似异步，实际不可靠。

`diagnosis-resources:v2` 因此采用同步执行：POST message 创建 running run，释放创建事务，完成
GraphRAG 与顶层 workflow，再用新事务原子写 completed result 和全部事件。响应返回时 run 通常已
completed，但 GET run/events 可以稳定复读 PostgreSQL 结果，为后续真正后台化保留资源契约。
...
执行模式仍是 synchronous，没有 queued/cancelled、后台 worker、重试 API、session 列表或分页；
这些能力需要可靠持久化调度后再升级状态契约。checkpoint 当前保存最新公开快照，尚未实现超大
会话的可配置滚动裁剪/归档，也不等同于 LangGraph 框架内部逐节点中断恢复。
...
`portfolio-eval-manifest:v19` 把这些层与 Golden 诊断基线的 suite ID、来源契约、结果文档、PostgreSQL 前置、受限 pytest target 和
已审核指标快照集中到 `data/evals/portfolio_eval_manifest.json`。它只汇总现有实测，不把不同层计算
成一个没有统计意义的总准确率。

`app/evaluation/portfolio.py` 实现 `portfolio-eval-run:v19`。测试 target 必须是仓库内 `tests/*.py`
文件或测试节点，manifest 不能加入任意 flags；真实执行使用当前 Python 解释器和
`subprocess.run(shell=False)`，stdout/stderr 被捕获后只在失败层提供截断摘要。运行器不会输出环境变量、
数据库 URL、Prompt、Thought 或供应商响应体。默认文件和 CLI 使用五层 v19；v1 精确四层与 v2/v3/v4/v5/v6/v7/v8/v9/v10/v11/v12/v13/v14/v15/v16/v17/v18
Golden v1/v2/v3/v4/v5/v6/v7/v8/v9/v10/v11/v12/v13/v14/v15/v16/v17 来源仍可兼容读取，旧结果不会被静默解释为新完整运行。

调用链如下：

```text
python -m app.evaluation
  -> load and validate portfolio-eval-manifest:v19
  ...
  -> render portfolio-eval-run:v19 JSON to stdout
```
...
`app/evaluation/golden_diagnosis.py` 实现 `golden-diagnosis-eval:v18`。它依赖可替换异步 runner，但评分
...
`golden-case:v7` 的 `case_category` 当前为 8/7/4/3/3。25 条案例使用 15 个 Fixture；三条记忆案例
...
当前 manifest 是四层小样本消融加一层 25 条 Golden 确定性回归，不是产品规划中的完整 28 条诊断
Golden Cases。Golden 报告固定 `target_case_count=28`、`case_coverage_rate=25/28` 和
`target_coverage_complete=false`；确定性 runner 按标注选择 Action 和根因，所以满分不代表模型级
...
- `golden-case:v7` Schema/位点/倾斜/参数反证、补参/降级/跨组件门禁、记忆与冲突标注，以及 `golden-diagnosis-eval:v18` 二十五条案例评测。
- `portfolio-eval-manifest:v19` 五层受限 pytest 入口、v1–v18 兼容读取、指标发布门禁，以及 `portfolio-eval-run:v19` 单命令 JSON 汇总。
