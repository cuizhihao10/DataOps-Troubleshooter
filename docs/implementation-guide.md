...
`portfolio-eval-manifest:v1` 把 suite ID、来源契约、结果文档、PostgreSQL 前置、受限 pytest target 和
已审核指标快照集中到 `data/evals/portfolio_eval_manifest.json`。它只汇总现有实测，不把不同层计算
成一个没有统计意义的总准确率。

`app/evaluation/portfolio.py` 实现 `portfolio-eval-run:v1`。测试 target 必须是仓库内 `tests/*.py`
文件或测试节点，manifest 不能加入任意 flags；真实执行使用当前 Python 解释器和
`subprocess.run(shell=False)`，stdout/stderr 被捕获后只在失败层提供截断摘要。运行器不会输出环境变量、
数据库 URL、Prompt、Thought 或供应商响应体。

调用链如下：

```text
python -m app.evaluation
  -> load and validate portfolio-eval-manifest:v1
  ...
  -> render portfolio-eval-run:v1 JSON to stdout
```
...
`complete` 表示没有 skipped/blocked，`all_suites_passed` 只有四层全 passed 才为真。
...
`.venv\Scripts\python -m app.evaluation --skip-postgres` 只运行 History/Auditor 两层真实 LangGraph
评测，用于无 Docker 环境的快速反馈；即使退出码为零，JSON 仍明确 `complete=false` 和
`all_suites_passed=false`。`tests/integration/test_portfolio_evaluation_cli.py` 会真实启动该 CLI，并验证
两个 skipped suite 没有 metrics。

当前 manifest 是四层小样本消融目录，不是产品规划中的完整 28 条诊断 Golden Cases。现有
`data/fixtures/golden_cases.json` 仍只有 5 条格式/场景基线；统一运行器没有将它们虚报为 28 条，
也没有给出模型级意图识别、Top-1 根因或 P95 成绩。
...
| `data/evals/*.json` | 标准评测数据不能加入非标准注释。 | 四层 Eval Schema、Portfolio Manifest、快速加载测试、本文档和五份实测报告。 |
...
- `portfolio-eval-manifest:v1` 四层受限 pytest 入口、指标发布门禁，以及 `portfolio-eval-run:v1` 单命令 JSON 汇总。
