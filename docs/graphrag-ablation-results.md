...
| 查询 | `sync backlog` |
| 知识数据 | `graph-seed:v10`，47 个节点、61 条边；旧知识保留 v1–v9 source，新增结算链与源端授权租约过期知识使用 v10 source |
...
路径。v10 的结算授权链没有改变该固定查询的候选截断结果，因此上述 5634/7/4 快照继续由 PostgreSQL
...
这些断言验证 v2–v10 知识真实入图；新增候选可以诚实改变 Bundle 组成，但没有被包装成既有主键冲突
消融指标的额外增益。
