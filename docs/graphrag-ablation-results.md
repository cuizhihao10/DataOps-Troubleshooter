...
| 查询 | `sync backlog` |
| 知识数据 | `graph-seed:v8`，33 个节点、41 条边；旧知识保留 v1–v7 source，新增客户分群数据倾斜拓扑使用 v8 source |
...
v8 的独立客户分群节点不会进入该固定查询的预算内，因此快照继续保持 5881 字节/8 节点，
...
LTS→BDS→数据集交付链及 BDS→长尾→倾斜入口。这些断言验证 v2–v8 知识真实入图，但没有被混入
既有主键冲突消融指标制造额外增益。
