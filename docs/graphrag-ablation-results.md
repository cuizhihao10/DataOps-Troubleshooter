...
| 查询 | `sync backlog` |
| 知识数据 | `graph-seed:v6`，27 个节点、29 条边；旧知识保留 v1–v5 source，新增客户画像任务拓扑使用 v6 source |
...
在默认 6000 字节、8 节点、4 路径预算下，Bundle 主体实测使用 4962 字节，选择 7 个去重节点和 4 条完整路径；另有 6 条路径因路径数或字节预算被明确列入 `omitted_path_ids`。关键两跳因果路径完整进入 Bundle，没有截断节点正文或丢失任一条边。

这些数字只适用于上述固定数据、Provider 和代码。增加案例、替换 embedding 模型或修改预算后必须重新运行 PostgreSQL 集成测试并更新记录，不能将本次结果外推为通用准确率提升。

v6 新增任务拓扑使 `sync backlog` 固定查询的 Bundle 从 v5 的 4477 字节/6 节点/3 条省略路径变为
4962 字节/7 节点/6 条省略路径，但仍保留 4 条完整路径并低于 6000 字节预算。同一 PostgreSQL 测试还会
...
`flashsync_customer_profile_delta` 查询任务依赖链及 Schema 症状入口。这些断言验证 v2–v6 知识真实
入图，但没有被混入既有主键冲突消融指标来制造额外增益。
