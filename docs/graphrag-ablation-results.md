...
| 查询 | `sync backlog` |
| 知识数据 | `graph-seed:v2`，14 个节点、15 条边；旧知识保留 v1 source，新增 LTS 参数链使用 v2 source |
...
v2 新增知识没有改变 `sync backlog` 消融案例的 4477 字节 Bundle 实测值；同一 PostgreSQL 测试还会
独立查询 `LTS 参数校验失败 partition_date`，并要求递归 CTE 返回“症状 → 根因 → 解决方案”两跳路径。
这项路径断言用于验证新增单组件知识真实入图，但没有被混入既有主键冲突消融指标来制造额外增益。
