...
| 查询 | `sync backlog` |
| 知识数据 | `graph-seed:v3`，17 个节点、17 条边；旧知识保留 v1/v2 source，新增 BDS 倾斜链使用 v3 source |
...
v3 新增知识没有改变 `sync backlog` 消融案例的 4477 字节 Bundle 实测值；同一 PostgreSQL 测试还会
分别查询 `LTS 参数校验失败 partition_date` 和 `BDS 执行阶段长尾 数据倾斜`，要求递归 CTE 返回两条
独立“症状 → 根因 → 解决方案”路径。这些断言用于验证 v2/v3 单组件知识真实入图，但没有被混入既有
主键冲突消融指标来制造额外增益。
