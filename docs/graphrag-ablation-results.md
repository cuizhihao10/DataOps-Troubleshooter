...
| 查询 | `sync backlog` |
| 知识数据 | `graph-seed:v12`，54 个节点、71 条边；旧知识保留 v1–v10 source，新增订单履约链与水位线时区错配知识使用 v11 source；v12 只为 8 个 `solution` 节点补 `remediation_risk_level` 声明，不改拓扑 |
...
在默认 6000 字节、8 节点、4 路径预算下，Bundle 主体实测使用 5876 字节，选择 7 个去重节点和 4 条完整路径；另有 5 个节点和 4 条路径因节点数、路径数或字节预算被明确列入 omitted IDs。关键两跳主键冲突因果路径完整进入 Bundle，没有截断节点正文或丢失任一条边。
...
路径。v10 的结算授权链和 v11 的订单水位线链都没有改变该固定查询的候选截断结果。`document-retrieval:v1`
接入后计费主体多了一个规范包装键 `,"selected_documents":[]`（24 字节），因此同一查询的实测快照变为
5658 字节/7 节点/4 路径、省略 5 个节点和 4 条路径：图证据的选择结果逐条未变，只是字节计费包含了
共享预算的第三类证据入口。`graph-seed:v12` 让每个紧凑节点多出一个 `remediation_risk_level` 字段
（6 个事实节点各 31 字节的 `null`、1 个方案节点 32 字节的 `"medium"`），快照因此变为
5876 字节/7 节点/4 路径、省略 5 个节点和 4 条路径。空值也真实进入 Prompt，所以它照实计费，而不是
为了保住旧数字在计费里排除 `null`。上述 5876/7/4 快照继续由 PostgreSQL
