...
def test_implementation_guide_covers_current_technology_boundaries() -> None:
    ...
    assert "golden-diagnosis-eval:v16" in guide
    assert "GoldenEvidenceConflictExpectation" in guide
    assert "统一作品集评测 manifest 与单命令运行器" in guide
    assert "portfolio-eval-run:v17" in guide
    assert "graph-seed:v6" in guide
    assert "27 节点/29 边" in guide
...
def test_portfolio_eval_report_documents_publish_gate_and_incomplete_golden_scope() -> None:
    """确认统一报告锁定 passed 才发布指标、快速模式不完整和 23/28 条边界。

    该门禁防止 CLI 失败后仍宣传 manifest 快照，也防止把四层消融或现有 23 条案例相加后冒充
    28 条诊断 Golden Case 成绩；完整命令、快速命令和十九个指标范围都必须可见。
    """

    report = Path("docs/portfolio-eval-results.md").read_text(encoding="utf-8")

    assert "portfolio-eval-manifest:v17" in report
    assert "portfolio-eval-run:v17" in report
    ...
    assert "有 23 条案例、使用 13 个场景" in report
    assert "成功响应证据冲突安全处置率" in report
    assert "产品目标是 28 条" in report
    assert "不能外推为真实 LLM" in report


def test_golden_diagnosis_report_documents_scoring_and_twenty_three_case_boundary() -> None:
    """确认 Golden 报告解释 Schema 跨组件传播、数量闭环和 23/28 未完成资格。
    ...
    assert "golden-diagnosis-eval:v16" in report
    assert "23/28 = 82.14%" in report
    ...
    assert "工具尝试成功率 | 89.86%" in report
    ...
    assert "跨组件故障 | 5 | 10 | 5" in report
    assert "15 条适用案例、21 条必要路径" in report
    ...
    assert "graph-seed:v6" in report
...
def test_diagnosis_resource_contract_documents_persistence_events_and_failure_semantics() -> None:
    ...
    assert "portfolio-eval-manifest:v17" in prompt_contract
    assert "portfolio-eval-run:v17" in prompt_contract
...
def test_ablation_report_labels_measured_values_and_honest_zero_gain() -> None:
    ...
    assert "graph-seed:v6" in report
    assert "27 个节点、29 条边" in report
    assert "4962 字节" in report
