...
def test_implementation_guide_covers_current_technology_boundaries() -> None:
    ...
    assert "golden-diagnosis-eval:v19" in guide
    assert "GoldenEvidenceConflictExpectation" in guide
    assert "统一作品集评测 manifest 与单命令运行器" in guide
    assert "portfolio-eval-run:v20" in guide
    assert "graph-seed:v9" in guide
    assert "40 节点/51 边" in guide
...
def test_portfolio_eval_report_documents_publish_gate_and_incomplete_golden_scope() -> None:
    """确认统一报告锁定 passed 才发布指标、快速模式不完整和 26/28 条边界。

    该门禁防止 CLI 失败后仍宣传 manifest 快照，也防止把四层消融或现有 26 条案例相加后冒充
    28 条诊断 Golden Case 成绩；完整命令、快速命令和十九个指标范围都必须可见。
    """

    report = Path("docs/portfolio-eval-results.md").read_text(encoding="utf-8")

    assert "portfolio-eval-manifest:v20" in report
    assert "portfolio-eval-run:v20" in report
    ...
    assert "有 26 条案例、使用 16 个场景" in report
    assert "成功响应证据冲突安全处置率" in report
    assert "产品目标是 28 条" in report
    assert "不能外推为真实 LLM" in report


def test_golden_diagnosis_report_documents_scoring_and_twenty_six_case_boundary() -> None:
    """确认 Golden 报告解释写入限流三组件传播、反证闭环和 26/28 未完成资格。
    ...
    assert "golden-diagnosis-eval:v19" in report
    assert "26/28 = 92.86%" in report
    ...
    assert "工具尝试成功率 | 91.95%" in report
    ...
    assert "跨组件故障 | 8 | 10 | 2" in report
    assert "18 条适用案例、30 条必要路径" in report
    ...
    assert "graph-seed:v9" in report
...
def test_diagnosis_resource_contract_documents_persistence_events_and_failure_semantics() -> None:
    ...
    assert "portfolio-eval-manifest:v20" in prompt_contract
    assert "portfolio-eval-run:v20" in prompt_contract
