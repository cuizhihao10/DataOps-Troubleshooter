...
def test_implementation_guide_covers_current_technology_boundaries() -> None:
    ...
    assert "golden-case:v2" in guide
    assert "golden-diagnosis-eval:v2" in guide
    assert "统一作品集评测 manifest 与单命令运行器" in guide
    assert "portfolio-eval-run:v3" in guide
...
def test_portfolio_eval_report_documents_publish_gate_and_incomplete_golden_scope() -> None:
    ...
    assert "portfolio-eval-manifest:v3" in report
    assert "portfolio-eval-run:v3" in report
...
def test_golden_diagnosis_report_documents_scoring_and_five_of_twenty_eight_boundary() -> None:
    ...
    assert "golden-case:v2" in report
    assert "golden-diagnosis-eval:v2" in report
...
def test_diagnosis_resource_contract_documents_persistence_events_and_failure_semantics() -> None:
    ...
    assert "portfolio-eval-manifest:v3" in prompt_contract
    assert "portfolio-eval-run:v3" in prompt_contract
