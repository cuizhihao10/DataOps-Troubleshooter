...
def test_all_scenarios_load_and_match_golden_cases() -> None:
    ...
    parameter_case = next(
        case
        for case in golden_cases
        if case.case_id == "golden_lts_invalid_partition_parameter_single"
    )
    assert parameter_case.required_tools == [
        ToolName.LTS_GET_TASK_STATUS,
        ToolName.LTS_GET_TASK_LOG,
        ToolName.LTS_GET_DEPENDENCY_TOPOLOGY,
    ]
    assert parameter_case.required_fault_paths[0].required_relation_types == [
        "CAUSED_BY",
        "RESOLVED_BY",
    ]
    ...
    checkpoint_case = next(
        case
        for case in golden_cases
        if case.case_id == "golden_flashsync_checkpoint_regression_single"
    )
    assert checkpoint_case.required_tools == [
        ToolName.FLASHSYNC_GET_SYNC_DELAY,
        ToolName.FLASHSYNC_GET_SYNC_LOG,
        ToolName.FLASHSYNC_CHECK_CONSISTENCY,
    ]
    assert checkpoint_case.required_fault_paths[0].required_node_ids == [
    ...
    schema_cross_case = next(
        case
        for case in golden_cases
        if case.case_id == "golden_cross_customer_profile_schema_propagation"
    )
    assert schema_cross_case.required_tools == [
        ToolName.LTS_GET_TASK_STATUS,
        ToolName.LTS_GET_DEPENDENCY_TOPOLOGY,
        ToolName.BDS_GET_TASK_STATUS,
        ToolName.BDS_GET_TABLE_INFO,
    ...
    assert [path.path_label for path in schema_cross_case.required_fault_paths] == [
        "customer_profile_task_dependency_chain",
        "customer_profile_schema_manifest_chain",
        "flashsync_schema_mapping_solution_chain",
    ]
    assert schema_cross_case.required_fault_paths[0].required_node_ids == [
    ...
    skew_cross_case = next(
        case
        for case in golden_cases
        if case.case_id == "golden_cross_lts_blocked_by_bds_data_skew"
    )
    assert skew_cross_case.required_tools == [
        ToolName.LTS_GET_TASK_STATUS,
        ToolName.LTS_GET_TASK_LOG,
        ToolName.LTS_GET_DEPENDENCY_TOPOLOGY,
        ToolName.BDS_GET_TASK_STATUS,
    ...
    assert [path.path_label for path in skew_cross_case.required_fault_paths] == [
        "customer_segment_delivery_chain",
        "customer_segment_skew_manifest_chain",
        "bds_data_skew_solution_chain",
    ]
    assert skew_cross_case.required_fault_paths[0].required_node_ids == [
        "task_lts_customer_segment_report",
        "task_bds_customer_segment_aggregate",
        "dataset_dws_customer_segment_hourly",
    ]
    assert len(skew_cross_case.required_evidence_sources) == 6
    throttle_cross_case = next(
        case
        for case in golden_cases
        if case.case_id == "golden_cross_lts_bds_flashsync_target_write_throttle"
    )
    assert throttle_cross_case.required_tools == [
        ToolName.LTS_GET_TASK_STATUS,
        ToolName.LTS_GET_DEPENDENCY_TOPOLOGY,
        ToolName.BDS_GET_TASK_STATUS,
        ToolName.BDS_GET_TABLE_INFO,
    ...
    assert [path.path_label for path in throttle_cross_case.required_fault_paths] == [
        "revenue_task_dependency_chain",
        "payment_target_throttle_manifest_chain",
        "flashsync_target_throttle_solution_chain",
    ]
    assert throttle_cross_case.required_fault_paths[0].required_node_ids == [
        "task_lts_revenue_dashboard",
        "task_bds_revenue_aggregate",
        "task_flashsync_payment_delta",
    ]
    assert throttle_cross_case.allowed_root_causes[0] == "FlashSync 目标端写入限流"
    assert len(throttle_cross_case.required_evidence_sources) == 6
    source_auth_cross_case = next(
        case
        for case in golden_cases
        if case.case_id == "golden_cross_lts_bds_flashsync_source_authorization_expired"
    )
    assert source_auth_cross_case.required_tools == [
        ToolName.LTS_GET_TASK_STATUS,
        ToolName.LTS_GET_DEPENDENCY_TOPOLOGY,
        ToolName.BDS_GET_TASK_STATUS,
        ToolName.BDS_GET_TABLE_INFO,
    ...
    assert [path.path_label for path in source_auth_cross_case.required_fault_paths] == [
        "settlement_task_dependency_chain",
        "settlement_source_auth_manifest_chain",
        "flashsync_source_auth_solution_chain",
    ]
    assert source_auth_cross_case.required_fault_paths[0].required_node_ids == [
