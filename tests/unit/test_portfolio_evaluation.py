...
def test_portfolio_manifest_v1_remains_readable_with_exact_legacy_four_suites() -> None:
    """验证升级默认 v22 后仍可读取精确四层的历史 v1 manifest。
...
def test_portfolio_manifest_v2_requires_the_original_golden_v1_source() -> None:
    ...
    测试从当前 v16 JSON 删除后续指标并回写两个旧 contract；若只修改版本却保留 Golden v2
...
def test_portfolio_manifest_v5_preserves_eleven_case_memory_snapshot() -> None:
    ...
    测试从当前 v16 payload 回写旧来源与覆盖值并删除新增指标，确认可读取；若保留 v6 冲突指标则
...
def test_portfolio_manifest_v6_preserves_twelve_case_conflict_snapshot() -> None:
    ...
    v6 与当前 v22 的指标集合相同，但 Golden 来源和覆盖快照不同；测试先回写 12 条版本并确认可读，
...
def test_portfolio_manifest_v7_preserves_thirteen_case_lts_bds_snapshot() -> None:
    ...
    v7 与当前 v22 共享指标集合和 Golden Case Schema，但来源评测版本及覆盖率不同；回写 13 条快照
...
def test_portfolio_manifest_v13_preserves_nineteen_case_lts_graph_snapshot() -> None:
    """验证历史 v13 绑定 Golden v12 与 19/28 的 LTS 参数 GraphRAG 快照。

    回写 v13 来源和覆盖率后应通过；注入 v14 的 20/28 数值必须失败。该门禁让 LTS 参数反证与
    BDS 数据倾斜反证保持两个独立版本，防止旧报告被解释成已经包含 graph-seed:v3。
    """

    payload = json.loads(MANIFEST_PATH.read_text(encoding="utf-8"))
    # 复用当前完整指标集合，仅回写版本绑定字段，贴近用户读取历史 manifest 的实际路径。
    payload["contract_id"] = "portfolio-eval-manifest:v13"
    golden_suite = next(
        suite for suite in payload["suites"] if suite["suite_id"] == "golden_diagnosis_baseline"
    )
    ...
    coverage["treatment_label"] = "measured_scripted_19_cases"
    coverage["treatment_value"] = 0.6786
    coverage["delta"] = -0.3214
    ...
    # 合同版本不变时替换成 v14 数值必须失败，避免覆盖率与 source contract 互相错位。
    coverage["treatment_value"] = 0.7143
    coverage["delta"] = -0.2857
    with pytest.raises(ValidationError, match="requires Golden coverage snapshot"):
        PortfolioEvalManifest.model_validate(payload)


def test_portfolio_manifest_v14_preserves_twenty_case_bds_skew_snapshot() -> None:
    """验证历史 v14 绑定 Golden v13 与 20/28 的 BDS 倾斜 GraphRAG 快照。

    v14 回写后必须可读，换成 v15 的 21/28 覆盖则必须失败。该版本门禁防止 BDS 中风险倾斜方案和
    FlashSync 高风险检查点恢复被合并解释，也防止旧结果被声称已包含 graph-seed:v4。
    """
    ...
    coverage["treatment_label"] = "measured_scripted_20_cases"
    coverage["treatment_value"] = 0.7143
    coverage["delta"] = -0.2857

    manifest = PortfolioEvalManifest.model_validate(payload)
    assert manifest.contract_id == "portfolio-eval-manifest:v14"

    # 版本未提升却注入 21/28 数值必须失败，保证报告案例集和 Golden source 同步演进。
    coverage["treatment_value"] = 0.75
    coverage["delta"] = -0.25
    with pytest.raises(ValidationError, match="requires Golden coverage snapshot"):
        PortfolioEvalManifest.model_validate(payload)


def test_portfolio_manifest_v15_preserves_twenty_one_case_checkpoint_snapshot() -> None:
    """验证历史 v15 绑定 Golden v14 与 21/28 的检查点回退快照。

    v15 的来源和覆盖回写后应通过；替换为 v16 的 22/28 必须失败。该门禁让 high 风险检查点恢复与
    medium 风险 Schema 映射修复保持独立版本，并防止旧报告被解释成已补齐单组件 8/8。
    """

    payload = json.loads(MANIFEST_PATH.read_text(encoding="utf-8"))
    # 保留当前 suite/metric 结构，只回写版本绑定字段，复现真实历史报告读取方式。
    payload["contract_id"] = "portfolio-eval-manifest:v15"
    golden_suite = next(
        suite for suite in payload["suites"] if suite["suite_id"] == "golden_diagnosis_baseline"
    )
    ...
    coverage["treatment_label"] = "measured_scripted_21_cases"
    coverage["treatment_value"] = 0.75
    coverage["delta"] = -0.25

    manifest = PortfolioEvalManifest.model_validate(payload)
    assert manifest.contract_id == "portfolio-eval-manifest:v15"

    # v15 合同下的 v16 数值必须失败，保证版本与 Golden source/案例集不可分离。
    coverage["treatment_value"] = 0.7857
    coverage["delta"] = -0.2143
    with pytest.raises(ValidationError, match="requires Golden coverage snapshot"):
        PortfolioEvalManifest.model_validate(payload)
...
def test_portfolio_manifest_v16_preserves_twenty_two_case_schema_snapshot() -> None:
    ...
    当前 v22 JSON 回写 v16 后必须可读；若注入 v17 的 23/28 覆盖也必须失败。这个版本门禁让
    “补齐单组件 8/8”和随后新增的三组件传播案例保持独立，旧报告不会被误解为已含 v6 任务拓扑。
    """

    payload = json.loads(MANIFEST_PATH.read_text(encoding="utf-8"))
    payload["contract_id"] = "portfolio-eval-manifest:v16"
    golden_suite = next(
        suite for suite in payload["suites"] if suite["suite_id"] == "golden_diagnosis_baseline"
    )
    ...
    coverage["treatment_label"] = "measured_scripted_22_cases"
    coverage["treatment_value"] = 0.7857
    coverage["delta"] = -0.2143

    manifest = PortfolioEvalManifest.model_validate(payload)
    assert manifest.contract_id == "portfolio-eval-manifest:v16"

    # v16 合同拒绝 v17 覆盖值，防止跨组件 Schema 传播被无版本地塞回历史结果。
    coverage["treatment_value"] = 0.8214
    coverage["delta"] = -0.1786
    with pytest.raises(ValidationError, match="requires Golden coverage snapshot"):
        PortfolioEvalManifest.model_validate(payload)
...
def test_portfolio_manifest_v17_preserves_twenty_three_case_schema_propagation_snapshot() -> None:
    ...
    当前 v22 JSON 回写 v17、Golden v16 和 23 条覆盖后必须可读；若仍使用 v18 的 24/28 快照则必须
    失败。这个门禁把 medium 风险 Schema 传播和 high 风险检查点传播拆成两个可审计版本。
    """

    payload = json.loads(MANIFEST_PATH.read_text(encoding="utf-8"))
    payload["contract_id"] = "portfolio-eval-manifest:v17"
    golden_suite = next(
        suite for suite in payload["suites"] if suite["suite_id"] == "golden_diagnosis_baseline"
    )
    ...
    coverage["treatment_label"] = "measured_scripted_23_cases"
    coverage["treatment_value"] = 0.8214
    coverage["delta"] = -0.1786

    manifest = PortfolioEvalManifest.model_validate(payload)
    assert manifest.contract_id == "portfolio-eval-manifest:v17"

    # v17 合同拒绝 v18 覆盖值，避免旧报告被解释为已经包含 BDS→FlashSync 检查点传播链。
    coverage["treatment_value"] = 0.8571
    coverage["delta"] = -0.1429
    with pytest.raises(ValidationError, match="requires Golden coverage snapshot"):
        PortfolioEvalManifest.model_validate(payload)
...
def test_portfolio_manifest_v18_preserves_twenty_four_case_checkpoint_snapshot() -> None:
    ...
    当前 v22 JSON 回写 v18、Golden v17 和 24 条覆盖后必须可读；随后注入 v19 的 25/28 覆盖必须
    失败。该门禁把 BDS→FlashSync 检查点传播与本切片的 LTS→BDS 数据倾斜传播分成两个可追溯版本。
    """

    payload = json.loads(MANIFEST_PATH.read_text(encoding="utf-8"))
    payload["contract_id"] = "portfolio-eval-manifest:v18"
    golden_suite = next(
        suite for suite in payload["suites"] if suite["suite_id"] == "golden_diagnosis_baseline"
    )
    ...
    coverage["treatment_label"] = "measured_scripted_24_cases"
    coverage["treatment_value"] = 0.8571
    coverage["delta"] = -0.1429

    manifest = PortfolioEvalManifest.model_validate(payload)
    assert manifest.contract_id == "portfolio-eval-manifest:v18"
...
def test_portfolio_manifest_v19_preserves_twenty_five_case_skew_snapshot() -> None:
    ...
    当前 v22 JSON 回写 v19、Golden v18 和 25 条覆盖后必须可读；再注入 v20 的 26/28 覆盖必须
...
def test_portfolio_manifest_v20_preserves_twenty_six_case_throttle_snapshot() -> None:
    """验证历史 v20 绑定 Golden v19 与 26/28 的目标写入限流传播快照。

    当前 v22 JSON 回写 v20、Golden v19 和 26 条覆盖后必须可读；再注入 v21 的 27/28 覆盖必须
    失败。该门禁阻止旧报告无版本地获得源端授权租约过期案例及 graph-seed:v10 安全轮换语义。
    """

    payload = json.loads(MANIFEST_PATH.read_text(encoding="utf-8"))
    payload["contract_id"] = "portfolio-eval-manifest:v20"
    golden_suite = next(
        suite for suite in payload["suites"] if suite["suite_id"] == "golden_diagnosis_baseline"
    )
    ...
    coverage["treatment_label"] = "measured_scripted_26_cases"
    coverage["treatment_value"] = 0.9286
    coverage["delta"] = -0.0714
    ...
    # v20 只能解释 26 条案例；第 27 条授权过期传播必须使用新的 v21 契约。
    coverage["treatment_value"] = 0.9643
    coverage["delta"] = -0.0357
    with pytest.raises(ValidationError, match="requires Golden coverage snapshot"):
        PortfolioEvalManifest.model_validate(payload)


def test_portfolio_manifest_v21_preserves_twenty_seven_case_authorization_snapshot() -> None:
    ...
    payload = json.loads(MANIFEST_PATH.read_text(encoding="utf-8"))
    payload["contract_id"] = "portfolio-eval-manifest:v21"
    golden_suite = next(
        suite for suite in payload["suites"] if suite["suite_id"] == "golden_diagnosis_baseline"
    )
    ...
    coverage["treatment_label"] = "measured_scripted_27_cases"
    coverage["treatment_value"] = 0.9643
    coverage["delta"] = -0.0357

    manifest = PortfolioEvalManifest.model_validate(payload)
    assert manifest.contract_id == "portfolio-eval-manifest:v21"

    # v21 只能解释 27 条案例；第 28 条水位线时区错配必须使用新的 v22 契约。
    coverage["treatment_value"] = 1.0
    coverage["delta"] = 0.0
    with pytest.raises(ValidationError, match="requires Golden coverage snapshot"):
        PortfolioEvalManifest.model_validate(payload)
