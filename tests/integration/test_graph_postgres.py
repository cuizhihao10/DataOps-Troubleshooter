...
async def test_postgres_graph_seed_search_expansion_and_key_edge_ablation() -> None:
    ...
    try:
        ...
        async with factory() as session:
            ...
            # 5876 = 旧的 5658 加上 7 个节点新增的 `remediation_risk_level` 字段：6 个事实节点各写
            # `,"remediation_risk_level":null`（31 字节），方案节点写 `"medium"`（32 字节）。空值也
            # 真实进入 Prompt（Renderer 用 model_dump 不排除 None），所以它必须照实计费，不能为了
            # 保住旧快照而在计费里偷偷排除 null。图证据的选择结果逐条未变（仍 7 节点/4 路径/省略
            # 5+4），因此这里只更新快照而不是重新解释排序。
            assert evidence_bundle.used_bytes == 5876
