...
def test_portfolio_cli_fast_mode_runs_three_suites_and_marks_report_incomplete() -> None:
    ...
    子进程 stdout 必须是可解析的 `portfolio-eval-run:v15` JSON；passed 层发布当前已验证指标，
    ...
    assert report["contract_id"] == "portfolio-eval-run:v15"
    assert report["manifest_contract_id"] == "portfolio-eval-manifest:v15"
