...
两个真实 LangGraph 评测节点。GraphRAG/记忆层必须明确 skipped，证明统一 JSON 不把旧数据库数字
...
def test_portfolio_cli_fast_mode_runs_two_suites_and_marks_report_incomplete() -> None:
    """验证快速 CLI 返回零退出码、两层 passed、两层 skipped 和不完整标记。

    子进程 stdout 必须是可解析的 `portfolio-eval-run:v1` JSON；passed 层合计发布五个已验证指标，
    ...
    assert report["contract_id"] == "portfolio-eval-run:v1"
    assert report["manifest_contract_id"] == "portfolio-eval-manifest:v1"
    ...
    assert statuses.count(SuiteExecutionStatus.PASSED.value) == 2
    assert statuses.count(SuiteExecutionStatus.SKIPPED.value) == 2
    assert sum(len(suite["metrics"]) for suite in report["suites"]) == 5
    assert all(
        suite["metrics"] == []
