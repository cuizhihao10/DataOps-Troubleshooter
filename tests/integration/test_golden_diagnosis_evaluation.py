...
async def test_twenty_seven_golden_cases_produce_versioned_measured_diagnosis_baseline() -> None:
    ...
    missing_log_result = next(
        result
        for result in report.cases
        if result.case_id == "golden_ambiguous_flashsync_missing_causal_log"
    )
    assert missing_log_result.executed_tools == [
        "flashsync.get_sync_delay",
        "flashsync.get_sync_log",
        "flashsync.check_consistency",
    ]
    assert missing_log_result.observed_evidence_sources == [
    ...
    target_throttle_result = next(
        result
        for result in report.cases
        if result.case_id == "golden_cross_lts_bds_flashsync_target_write_throttle"
    )
    assert target_throttle_result.executed_tools == [
        "lts.get_task_status",
        "lts.get_dependency_topology",
        "bds.get_task_status",
        "bds.get_table_info",
    ...
    assert target_throttle_result.observed_evidence_sources == [
    ...
    assert target_throttle_result.actual_top1_root_cause == "FlashSync 目标端写入限流"
    assert target_throttle_result.matched_fault_path_labels == [
    ...
    checkpoint_result = next(
        result
        for result in report.cases
        if result.case_id == "golden_flashsync_checkpoint_regression_single"
    )
    assert checkpoint_result.executed_tools == [
        "flashsync.get_sync_delay",
        "flashsync.get_sync_log",
        "flashsync.check_consistency",
    ]
    assert checkpoint_result.observed_evidence_sources == [
    ...
    assert checkpoint_result.actual_top1_root_cause == "FlashSync 检查点位点回退"
    assert checkpoint_result.matched_fault_path_labels == [
        "flashsync_checkpoint_regression_solution_chain"
    ]
    assert checkpoint_result.actual_risk_level is RiskLevel.HIGH
    schema_result = next(
        result
        for result in report.cases
        if result.case_id == "golden_flashsync_schema_mapping_outdated_single"
    )
    assert schema_result.executed_tools == [
        "flashsync.get_sync_delay",
        "flashsync.get_sync_log",
        "flashsync.check_consistency",
    ]
    assert schema_result.observed_evidence_sources == [
    ...
    assert schema_result.actual_top1_root_cause == "FlashSync 字段映射版本滞后"
    assert schema_result.matched_fault_path_labels == [
    ...
    checkpoint_cross_result = next(
        result
        for result in report.cases
        if result.case_id == "golden_cross_bds_blocked_by_flashsync_checkpoint_regression"
    )
    assert checkpoint_cross_result.executed_tools == [
        "bds.get_task_status",
        "bds.get_task_log",
        "bds.get_table_info",
        "flashsync.get_sync_delay",
    ...
    assert checkpoint_cross_result.observed_evidence_sources == [
    ...
    assert checkpoint_cross_result.actual_top1_root_cause == "FlashSync 检查点位点回退"
    assert checkpoint_cross_result.matched_fault_path_labels == [
        "customer_status_checkpoint_delivery_chain",
        "customer_status_checkpoint_manifest_chain",
        "flashsync_checkpoint_regression_solution_chain",
    ]
    assert checkpoint_cross_result.actual_risk_level is RiskLevel.HIGH
            ...
            source_ids=[
                {
                    ...
                    "flashsync_checkpoint_regression_solution_chain": (
                        "synthetic_cross_chain_knowledge_v4"
                    ),
                    "flashsync_schema_mapping_solution_chain": (
