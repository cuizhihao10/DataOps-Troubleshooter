...
- 提供 15 个脱敏且确定性的合成场景，以及对应 Golden Case 格式。
...
- PostgreSQL + pgvector 保存 `graph-seed:v8` 的 33 个显式知识节点、41 条关系边和带 Provider 溯源的向量，支持全文/向量混合召回、五项可解释评分与 1–2 跳路径扩展。
...
- `golden-case:v7` 同时覆盖安全降级、反证、Schema、检查点与数据倾斜跨组件传播；当前 25 条案例使用 15 个脱敏 Fixture，单组件达到 8/8、跨组件达到 7/10。
- `golden-diagnosis-eval:v18` 要求 LTS 等待、BDS 长尾、数据倾斜直接日志与正常输入分区在六项证据中闭合，并使用 v8 传播路径；当前 25/28 确定性脚本满分不冒充真实 LLM 成绩。
- `portfolio-eval-run:v19` 通过 `python -m app.evaluation` 一次执行五层、19 个独立指标。
...
当前已完成全部 MCP 工具、GraphRAG 检索闭环、五项固定 runtime capabilities、Planner ReAct、独立 Auditor、长期案例记忆、五层小样本统一评测运行器、顶层诊断工作流、资源 API 和同 session checkpoint 追问恢复。默认模型 Provider 仍为 disabled；Golden 诊断层是 25 条确定性脚本回归基线，自动化测试不宣称已经调用付费模型或取得模型质量成绩。可靠后台 worker、LangGraph 逐节点中断恢复、模型级复杂语义对比和完整 28 条 Golden Case 仍待后续实现。
