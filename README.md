...
- `golden-case:v5` 新增成功 BDS 响应互相矛盾的机器可读边界，工具异常/证据冲突类别达到 3/3；当前 12 条案例复用 6 个脱敏 Fixture。
- `golden-diagnosis-eval:v5` 同时检查冲突来源完整观察、禁止根因零命中、无根因输出和不确定性公开；当前 12/28 确定性脚本满分不冒充真实 LLM 成绩。
- `portfolio-eval-run:v6` 通过 `python -m app.evaluation` 一次执行五层、19 个独立指标。
...
当前已完成全部 MCP 工具、GraphRAG 检索闭环、五项固定 runtime capabilities、Planner ReAct、独立 Auditor、长期案例记忆、五层小样本统一评测运行器、顶层诊断工作流、资源 API 和同 session checkpoint 追问恢复。默认模型 Provider 仍为 disabled；Golden 诊断层是 12 条确定性脚本回归基线，自动化测试不宣称已经调用付费模型或取得模型质量成绩。可靠后台 worker、LangGraph 逐节点中断恢复、模型级复杂语义对比和完整 28 条 Golden Case 仍待后续实现。
