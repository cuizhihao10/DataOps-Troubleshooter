...
def _candidate_from_result(result: ReportRunResult, *, now: datetime) -> CaseMemory:
    ...
    historical_case_ids = {item.case_id for item in report.similar_cases}
    # 相似案例 ID 只证明“参考过哪个先例”，不能成为新长期记忆证明自身根因的来源；否则重复合并
    # 会把 memory_id 递归写入自身 evidence_refs。只保留本次实时、知识节点或图路径引用。
    evidence_refs = _stable_unique(
        reference
        for reference in [*conclusion.evidence_refs, *report.evidence_refs]
        if reference not in historical_case_ids
    )
...
def _stable_unique(items: Iterable[str]) -> list[str]:
    ...
    接受列表或一次性 iterable，便于候选投影在过滤历史 case_id 时不创建第二份中间列表；dict 保留
    插入顺序。空输入返回空列表，是否允许为空由具体 CaseMemory 字段决定。
    """
