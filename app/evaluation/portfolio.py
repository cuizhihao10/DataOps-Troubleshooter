...
PORTFOLIO_EVAL_MANIFEST_CONTRACT_ID = "portfolio-eval-manifest:v14"
PORTFOLIO_EVAL_RUN_CONTRACT_ID = "portfolio-eval-run:v14"
...
class PortfolioEvalManifest(BaseModel):
    ...
    BDS 数据倾斜、正常总量反证及解决路径。版本与精确 suite、Golden 来源和覆盖快照绑定，旧 JSON
    不会被静默解释成当前完整运行。
    """
...
class PortfolioEvalRunReport(BaseModel):
    ...
    contract_id: Literal["portfolio-eval-run:v14"]
